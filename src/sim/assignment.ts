...
export class AssignmentTable {
  ...
  rebuild(world: World, field: RoadField, stats: CityStats): void {
      ...
      for (let ly = 0; ly < CHUNK_SIZE; ly++)
        for (let lx = 0; lx < CHUNK_SIZE; lx++) {
          ...
          const zone = zoneOfCode(code),
            ...
            tx = bx + lx,
            ty = by + ly;
          const road = firstRoad(world, tx, ty, level);
          if (!road) continue;
          const tierStats = stats.tiers[zone]?.[level - 1];
          const fillRatio =
            tierStats && tierStats.capacity > 0
              ? Math.max(0, Math.min(1, tierStats.filled / tierStats.capacity))
              : 0;
          ...
          if (zone === ZONE_R) homes.push(b);
          else {
            jobs.push(b);
            if (zone === ZONE_C) shops.push(b);
          }
        }
    }
    homes.sort(coordSort);
    jobs.sort(coordSort);
    shops.sort(coordSort);
    ...
    for (const home of homes) {
      ...
      candidates.sort(
        (a, b) =>
          jobScore(home.level, b.j.level, b.dist) - jobScore(home.level, a.j.level, a.dist) ||
          coordSort(a.j, b.j),
      );
      for (const { j, dist } of candidates) {
        if (left <= 0) break;
        const take = Math.min(left, j.remaining);
        if (take <= 0) continue;
        pushLink(out.jobs, j, take, dist);
        j.remaining -= take;
        left -= take;
      }
      out.unemployed = left;
      ...
      const shopCandidates = shopPool
        .sort(
          (a, b) =>
            shopScore(home.level, b.s.level, b.dist) - shopScore(home.level, a.s.level, a.dist) ||
            coordSort(a.s, b.s),
        )
        .slice(0, SHOP_LINKS_MAX);
      if (shopCandidates.length && filled > 0) {
        const base = Math.floor(filled / shopCandidates.length),
          rem = filled % shopCandidates.length;
        shopCandidates.forEach(({ s, dist }, i) =>
          pushLink(out.shops, s, base + (i < rem ? 1 : 0), dist),
        );
      }
  ...
  get(anchorTx: number, anchorTy: number): ParcelAssignment | undefined {
    return this.table.get(key(anchorTx, anchorTy));
  }
  jobForSlot(anchorTx: number, anchorTy: number, slot: number): DestLink | null {
    const a = this.get(anchorTx, anchorTy);
    if (!a || slot < 0) return null;
    let n = slot;
    for (const l of a.jobs) {
      if (n < l.count) return l;
      n -= l.count;
    }
    return null;
  }
  shopForSlot(anchorTx: number, anchorTy: number, slot: number, day: number): DestLink | null {
    const a = this.get(anchorTx, anchorTy);
    if (!a?.shops.length) return null;
    return a.shops[simHash(WORLD_SEED, anchorTx, anchorTy, slot ^ day) % a.shops.length];
  }
  *allLinks(): Generator<{ fromTx: number; fromTy: number; link: DestLink }> {
    for (const [k, a] of this.table) {
      const [x, y] = k.split(',').map(Number);
      for (const link of a.jobs) yield { fromTx: x, fromTy: y, link };
      for (const link of a.shops) yield { fromTx: x, fromTy: y, link };
    }
  }
}
...
function coordSort(a: { tx: number; ty: number }, b: { tx: number; ty: number }) {
  return a.ty - b.ty || a.tx - b.tx;
}
function pushLink(out: DestLink[], b: Building, count: number, dist: number) {
  if (count > 0) out.push({ tx: b.tx, ty: b.ty, count, level: b.level, zone: b.zone, dist });
}
function jobScore(homeLevel: number, jobLevel: number, dist: number) {
  return JOB_FIT[homeLevel - 1][jobLevel - 1] * 10000 - dist;
}
function shopScore(homeLevel: number, shopLevel: number, dist: number) {
  const levelBias = homeLevel === 3 ? shopLevel * 1800 : shopLevel * 500;
  const distanceWeight = homeLevel === 3 ? 15 : 40;
  return levelBias - dist * distanceWeight;
}
function firstRoad(world: World, tx: number, ty: number, span: number): [number, number] | null {
  for (const [x, y] of edgeNeighbors(tx, ty, span))
    if (world.getBuild(x, y) === Build.Road) return [x, y];
  return null;
}
