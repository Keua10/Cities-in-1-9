import { CHUNK_SIZE, MAX_ACTIVE_VEHICLES, SIM_RADIUS_CHUNKS, WORLD_SEED } from '../../core/constants';
...
export class TrafficSim {
  ...
  private activeCx=0;private activeCy=0;private initialized=false;private timeMs=0;private sampleMs=0;private spawnTokens=0;private spawnWindowMs=0;private spawnedInWindow=0;private readySpawns:{trip:Trip;route:Route}[]=[];private generation=0;private lastMacroTick=-1;
  constructor(private world:World,private macro:MacroSim,private congestion:CongestionMap,private assignment:AssignmentTable){this.router=new Router(world,congestion);this.citizens=new CitizenPool(world,macro,assignment);}
  ...
  update(dtMs:number):void{
    ...
    if(budget>0){const trips=this.citizens.collectTrips(this.macro.hourOfDay,dtMs,budget);for(const trip of trips){if(this.spawnTokens<1)break;this.spawnTokens-=1;this.queueTrip(trip);}}
  ...
  private spawnRoute(trip:Trip,route:Route):boolean{if(this.vehicles.length>=MAX_ACTIVE_VEHICLES||this.spawnedInWindow>=MAX_SPAWNS_PER_SEC)return false;let startIndex=0;if(!this.tileInside(route.tiles[0],route.tiles[1])){startIndex=-1;for(let i=0;i<route.tiles.length;i+=2){if(this.tileInside(route.tiles[i],route.tiles[i+1])){startIndex=i/2;break;}}if(startIndex<0){this.citizens.onTripFailed(trip);return true;}}
    const sx=route.tiles[startIndex*2],sy=route.tiles[startIndex*2+1];if(this.spawnBlocked(sx,sy))return false;const sliced=startIndex===0?route:this.router.sliceRoute(route,startIndex);const dir=sliced.tiles.length>=4?dirBetween(sliced.tiles[0],sliced.tiles[1],sliced.tiles[2],sliced.tiles[3]):0;const kind=trip.purpose===TripPurpose.Freight?VehicleKind.Truck:VehicleKind.Car;const vehicle:Vehicle={kind,tier:trip.tier,purpose:trip.purpose,route:sliced,routeIdx:0,tileT:0,lane:simHash(WORLD_SEED,trip.fromTx,trip.fromTy,this.macro.tick)&1,speed:0,dir,destTx:trip.toTx,destTy:trip.toTy};this.vehicles.push(vehicle);this.trips.set(vehicle,trip);this.spawnedInWindow++;return true;}
  ...
  private spawnBlocked(tx:number,ty:number){let n=0;for(const v of this.vehicles){const [x,y]=tileAt(v);if(x===tx&&y===ty)n++;}return n>=2;}
  private moveVehicles(dt:number){const occupancy=new Map<string,Vehicle[]>();for(const v of this.vehicles){const [x,y]=tileAt(v),k=`${x},${y},${v.lane}`;let a=occupancy.get(k);if(!a){a=[];occupancy.set(k,a);}a.push(v);}for(const a of occupancy.values())a.sort((x,y)=>y.tileT-x.tileT);
    const remove=new Set<Vehicle>();for(const v of this.vehicles){const points=v.route.tiles.length/2;if(v.routeIdx>=points-1){this.completeVehicle(v);remove.add(v);continue;}const [tx,ty]=tileAt(v),nextX=v.route.tiles[(v.routeIdx+1)*2],nextY=v.route.tiles[(v.routeIdx+1)*2+1];let target=VEHICLE_SPEED_TILES_PER_SEC*(v.kind===VehicleKind.Truck?TRUCK_SPEED_MUL:1);const gap=this.gapAhead(v,occupancy,nextX,nextY);if(gap<=MIN_GAP_TILES)target=0;else if(gap<DESIRED_GAP_TILES)target*=Math.max(0,(gap-MIN_GAP_TILES)/(DESIRED_GAP_TILES-MIN_GAP_TILES));if(hasSignal(this.world,nextX,nextY)&&!canEnter(nextX,nextY,v.dir,this.timeMs)&&v.tileT>.72)target=0;
  ...
  private gapAhead(v:Vehicle,occ:Map<string,Vehicle[]>,nextX:number,nextY:number){const [x,y]=tileAt(v);let best=Infinity;for(const o of occ.get(`${x},${y},${v.lane}`)??[]){if(o===v)continue;const d=o.tileT-v.tileT;if(d>0)best=Math.min(best,d);}for(const o of occ.get(`${nextX},${nextY},${v.lane}`)??[]){best=Math.min(best,1-v.tileT+o.tileT);}return best;}
