...
/**
 * 빈 땅에서 시작하는 새 도시의 시작 자금.
 *
 * 3.1단계에서 60,000 으로 잡았을 때 예산은 도로·지구·기본 시설뿐이었다. STEP 4
 * 에서 상하수도와 전기가 들어오면서 "첫 마을이 돌아가려면 반드시 지어야 하는 것"
 * 이 늘었다. 도시 레벨 1(마을)에서 열려 있는 것만 모아 보면
 *
 *   도로 180칸 x 12          =  2,160
 *   지구 350칸 x 8           =  2,800
 *   지하수 펌프 + 직접 방류구 =  5,000
 *   상수/하수관 240칸 x 12    =  2,880
 *   풍력 발전소 + 전선 120칸  =  7,160
 *   소공원 4채               =  2,400
 *   소방서 + 경찰서          =  8,500
 *   쓰레기 소각시설          = 10,000
 *                              -------
 *                              40,900
 *
 * 여기에 세수가 유지비를 넘기까지의 적자 구간(대략 30~40게임일)이 더해진다.
 * 60,000 으로는 그 구간을 넘기지 못하고 첫 소각시설에서 막힌다.
 *
 * STEP 5 에서 전기·상수·하수가 **입주의 전제 조건** 이 되면서(config/essentials.ts)
 * 이 목록은 "있으면 좋은 것" 이 아니라 첫날에 반드시 깔아야 하는 것이 되었다.
 * 80,000 으로 장기 주행을 돌리면 자금이 정확히 0 원까지 내려갔다 올라온다 —
 * 한 번만 어긋나도 성장이 멈춘다는 뜻이다. 120,000 이면 인프라 한 벌을 먼저
 * 깔고도 적자 구간을 버틴다. 여전히 계획 없이 도로부터 깔면 모자란다.
 */
 ...
 * 지형 수정 (수정사항 10).
 *
 * 한 칸 값이 아니라 **한 번에 고치는 넓이의 제곱** 으로 오른다:
 *
 *   칸당 = TERRAIN_COST x (1 + (칸수 - 1) / TERRAIN_SOFT_TILES)
 *
 * 그래서 도로 놓을 자리를 한두 칸 고르는 건 부담이 없고(400원, 1,200원),
 * 산을 통째로 깎는 건 감당이 안 된다(16칸 22,400원 · 100칸 70만 원).
 * 지형을 마음대로 주무를 수 있으면 도시 설계가 지형을 읽는 일이 아니게 된다.
 */
export const TERRAIN_COST = 400;
export const TERRAIN_SOFT_TILES = 6;
