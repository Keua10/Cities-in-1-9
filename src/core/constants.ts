...
/**
 * 3.1단계에서 추가. 시뮬레이션이 혼자 만든 변경(건물이 자란 것)만 쌓였을 때의
 * 저장 간격.
 *
 * 매크로 틱이 돌기 시작하면 도시는 학생이 아무것도 안 해도 계속 바뀐다.
 * 학생 조작과 같은 60초 간격으로 저장하면 도시 하나가 시간당 청크 문서를
 * 수백 개 쓰게 되고, 20명이면 Spark 하루 한도(쓰기 2만)를 수업 한 번에 넘긴다.
 * 그래서 "학생이 직접 고친 것" 과 "시뮬레이션이 지은 것" 의 저장 주기를 나눈다.
 * 어느 쪽이든 탭을 닫거나 로그아웃할 때는 즉시 저장하므로 작업이 사라지지 않는다.
 */
export const SAVE_SIM_INTERVAL_MS = 10 * 60_000;
