...
export class VehicleMesh{
  ...
  update(vehicles:readonly Vehicle[]){this.sorted.length=0;for(const v of vehicles)this.sorted.push(v);this.sorted.sort((a,b)=>depth(a)-depth(b));let q=0;for(const v of this.sorted){if(q>=MAX_ACTIVE_VEHICLES)break;const i=v.routeIdx*2,tx=v.route.tiles[i],ty=v.route.tiles[i+1];const ni=Math.min(i+2,v.route.tiles.length-2),nx=v.route.tiles[ni],ny=v.route.tiles[ni+1];const t=v.tileT;let wx=tileToWorldX(tx,ty)+(tileToWorldX(nx,ny)-tileToWorldX(tx,ty))*t;let wy=tileToWorldY(tx,ty,this.world.sampleHeight(tx,ty))+(tileToWorldY(nx,ny,this.world.sampleHeight(nx,ny))-tileToWorldY(tx,ty,this.world.sampleHeight(tx,ty)))*t;const dx=tileToWorldX(nx,ny)-tileToWorldX(tx,ty),dy=tileToWorldY(nx,ny,0)-tileToWorldY(tx,ty,0),len=Math.hypot(dx,dy)||1,lane=4;wx+=(-dy/len)*lane;wy+=(dx/len)*lane;const half=VEHICLE_CELL/2,x0=wx-half,x1=wx+half,y1=wy,y0=wy-VEHICLE_CELL;write(this.positions,q,[x0,y0,x1,y0,x1,y1,x0,y1]);const variant=simHash(WORLD_SEED,v.destTx,v.destTy,v.tier)%VEHICLE_VARIANTS;const [u0,v0,u1,v1]=this.atlas.uv(v.kind,v.dir,variant);write(this.uvs,q,[u0,v0,u1,v0,u1,v1,u0,v1]);q++;}for(;q<MAX_ACTIVE_VEHICLES;q++){write(this.positions,q,[0,0,0,0,0,0,0,0]);write(this.uvs,q,[0,0,0,0,0,0,0,0]);}this.geometry.getBuffer('aPosition').update();this.geometry.getBuffer('aUV').update();}
