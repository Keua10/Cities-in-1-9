...
export class IncidentLayer {
  ...
  draw(world: World, sim: DisasterSim, range: { cx0: number; cy0: number; cx1: number; cy1: number }): void {
    const g = this.graphics;
    g.clear();
    for (const e of sim.active) {
      const cx = chunkIndexOf(e.tx), cy = chunkIndexOf(e.ty);
      if (cx < range.cx0 || cx > range.cx1 || cy < range.cy0 || cy > range.cy1 ||
          !world.isExplored(cx, cy) || !sim.at(e.tx, e.ty, world)) continue;
