...
const source = await loadImage(readFileSync(new URL('./art/facilities-source.png', import.meta.url)));
...
const { data } = pixels, w = input.width, h = input.height, n = w * h;
...
for (let y = 600; y < 810; y++) for (let x = 1040; x < 1160; x++) {
  const i = y * w + x;
...
const labels = new Int32Array(n), components = [];
...
for (let i = 0; i < n; i++) {
  ...
  label++; head = 0; tail = 0; queue[tail++] = i; labels[i] = label;
  const c = { label, count: 0, x0: w, y0: h, x1: 0, y1: 0 };
  while (head < tail) {
    const p = queue[head++], x = p % w, y = Math.floor(p / w);
    c.count++; c.x0 = Math.min(c.x0, x); c.x1 = Math.max(c.x1, x);
...
if (components.length !== 7) throw new Error(`Expected seven cutouts, got ${JSON.stringify(components)}`);
// top park, then fire/police/park, then hospital/school/sports.
const sorted = [components.find(c => c.y1 < 300),
  ...components.filter(c => c.y0 < 300 && c.y1 >= 300).sort((a, b) => a.x0 - b.x0),
  ...components.filter(c => c.y0 >= 300).sort((a, b) => a.x0 - b.x0)];
const cells = [[0, 0, 64], [0, 64, 128], [128, 64, 128], [256, 64, 128],
  [0, 192, 192], [192, 192, 192], [384, 192, 192]];
const atlas = createCanvas(576, 384), out = atlas.getContext('2d');
out.imageSmoothingEnabled = false;
for (let k = 0; k < sorted.length; k++) {
  const c = sorted[k], [x, y, size] = cells[k];
  const cw = c.x1 - c.x0 + 1, ch = c.y1 - c.y0 + 1;
  const cut = createCanvas(cw, ch), cc = cut.getContext('2d'), p = cc.createImageData(cw, ch);
  for (let sy = 0; sy < ch; sy++) for (let sx = 0; sx < cw; sx++) {
    const i = (c.y0 + sy) * w + c.x0 + sx, dst = (sy * cw + sx) * 4;
...
const preview = createCanvas(1152, 768), pc = preview.getContext('2d');
pc.fillStyle = '#28323a'; pc.fillRect(0, 0, 1152, 768);
pc.imageSmoothingEnabled = false; pc.drawImage(atlas, 0, 0, 1152, 768);
